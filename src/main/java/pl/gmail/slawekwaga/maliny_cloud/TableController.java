...
public class TableController {
    ...
    @GetMapping("/table")
    public String tableMain(@RequestParam String dataID,
                            Model model,
        ...
        switch (dataID) {
            case "orders":
            {
                Pageable test= PageRequest.of(newPage-1,entries_per_page, Sort.by("idZamowienia"));
                Page<ViewOrder> pageObj= viewOrderRepo.findAll(test);
            ...
            case "clients": {
                ...
                Pageable test= PageRequest.of(newPage-1,entries_per_page, Sort.by("id"));
                Page<Klienci> pageObj = klienciRepo.findAll(test);
                model.addAttribute("data", pageObj);
                model.addAttribute("totalPages",pageObj.getTotalPages());
                if (controlAction!=null) {
                    if (controlAction.equals("last")|| newPage>=pageObj.getTotalPages()) {
    ...
    @Autowired
    public void setViewOrderRepo(ViewOrderRepo viewOrderRepo) {
        this.viewOrderRepo = viewOrderRepo;
    }

    @Autowired
    public void setKlienciRepo(KlienciRepo klienciRepo) {
        this.klienciRepo = klienciRepo;
    }
}
